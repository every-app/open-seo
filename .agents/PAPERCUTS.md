...
- [ ] `2026-07-19T17:57:02Z` — `codex` — The full `pnpm test` suite repeatedly times out `src/server/lib/dataforseo/client.test.ts`'s "skips billing in non-hosted mode" case at 5 seconds under suite concurrency, while that file passes in isolation in 1.7 seconds. Remove the test's avoidable delay or give this integration-style case appropriate timeout/concurrency isolation.
