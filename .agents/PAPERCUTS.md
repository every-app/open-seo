...
- [ ] `2026-08-25T03:13:05Z` — `codex` — A fresh managed `pnpm install` appends placeholder `allowBuilds` values such as `set this to true or false` to `pnpm-workspace.yaml`, then `pnpm ci:check` attempts another dependency purge and aborts without a TTY. Commit explicit build-policy values or document the required non-interactive install mode so validation starts from a clean tree.
