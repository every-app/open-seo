...
- [ ] `2026-07-28T10:41:00Z` — `claude` — On Windows, where `core.autocrlf=true` is the Git default, a fresh checkout fails `prettier --check .` (and so `pnpm ci:check`) on ~790 untouched files, because the working copy is CRLF while Prettier expects LF. The repo has no `.gitattributes`; adding one with `* text=auto eol=lf` would keep working copies LF on every platform.
