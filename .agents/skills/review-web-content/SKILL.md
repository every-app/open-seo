---
name: review-web-content
description: Write and review content for the SEOShark website (web/) — blog posts, guides, feature pages, FAQs. Distills the philosophy for on-brand, useful, accurate content. Use whenever adding or editing user-facing prose in web/content or web/src.
metadata:
  internal: true
---

# SEOShark Web Content
...
1. **Traceable truth.** Every capability claim, price, and screenshot is verifiable against the code, the fact sheet (`src/server/features/sam/product-fact-sheet.md`), or the live product. If you can't point to where it's true, it doesn't ship.
...
3. **Honest pricing, with its reasoning.** Quality SEO data is expensive everywhere. State the current plan and credit prices exactly as the pricing page and billing code define them; never round them off, and never call something simply "free" when it costs credits.
...
7. **Playbook terminology.** Call each approach within any SEOShark playbook a "strategy," never a "play." Use "workflow" for the steps readers execute; use "playbook" only for the complete collection.

## Questions to ask while reviewing

- If a reader trusted every claim and screenshot, then opened SEOShark right now, where would reality not match?
