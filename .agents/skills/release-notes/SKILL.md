---
name: release-notes
description: 'Cut a SEOShark release — bump the version, draft user-facing release notes from commits since the last tag, run a review + subagent-verification pass, and open a "release: vX.X.X" PR. Use when the user asks to prepare a release, bump the version, or write release notes.'
metadata:
  internal: true
---
...
- For each commit, fetch the PR body and author (`gh pr view <num> --repo bizztor/seoshark --json title,body,author`) — squash-commit subjects can be stale. The `(#NN)` in commit subjects references PRs on `bizztor/seoshark` (the `origin` remote); there is no other repo to fall back to. Commits with no `(#NN)` may still be an outside contribution (`gh pr list --repo bizztor/seoshark --state merged --author <login>`); check `git log --format=%an` for the author. Verify claims against the final code when a PR body and commit subject disagree (features get reverted before merge).
- Record the PR author's GitHub handle alongside each change so the bullet can credit them.

## 3. Draft the notes

Write `release-notes/v<version>.md`. Match the style of the most recent file in `release-notes/` when one exists; the format below is the contract either way. The notes are a scannable digest, not documentation: the whole file fits on one screen (roughly 15 lines including headings), and every line earns its place.
...
- **Credit the contributor.** End the bullet with `— thanks @handle` for outside contributors only — never for the maintainer's own PRs (`bizztor`). Credit goes on the top-level bullet, not sub-bullets. Multiple contributors: `— thanks @a, @b`.
- End with: `Full Changelog: https://github.com/bizztor/seoshark/compare/v<prev>...v<version>`

Curation — this is where the work is. Cut aggressively; the Full Changelog link covers the long tail:

- Only changes to the **product itself** — the app, the MCP tools, the SEO data/features someone running SEOShark actually uses. Litmus test per bullet: **would a self-hoster notice this while using the product?** Caring in the abstract (a new backend option, a raised cap) is not enough.
- Do NOT mention:
...
- Do not tag or publish the GitHub release — that happens after merge. After merge, run `pnpm release:publish`. It reads the version from `package.json` and publishes the matching `release-notes/v<version>.md` to `bizztor/seoshark`.
- The equivalent command is:

  ```sh
  gh release create v<version> \
    --repo bizztor/seoshark \
    --title v<version> \
