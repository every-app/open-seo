...
![OpenSEO keyword research for a Nashville kayak rental topic, sorted by opportunity score rather than volume, where a 10-search-a-month keyword scores highest and a 6,600-search keyword sits well below it](/library/keyword-research/keyword-difficulty-volume-openseo.png)

Sorting by opportunity rather than volume reorders the list immediately. In the run above, a term doing ten searches a month scores highest, and a term doing 6,600 sits several rows below it. A volume sort would have inverted that and sent the work to the wrong page.
