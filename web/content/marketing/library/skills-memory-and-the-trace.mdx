...
A skill is a file that tells the assistant how to do one job: what to pull, in what order, what to check, what the output looks like. SEOShark ships a set of them for the SEO jobs the MCP supports, and they are the reason the same prompt produces the same shape of work on Monday and on Thursday:

![The SEOShark agent skills listed in a terminal: competitive-landscape, competitor-analysis, keyword-clustering, keyword-research, link-prospecting, local-seo, seo-audit, seo-coach and seo-project-setup, each a SKILL.md file the assistant reads before doing that job](/library/ai-agent-seo/seoshark-agent-skills-seoshark.png "The SEOShark agent skills")
...
Connect the [SEOShark MCP](/docs/mcp) and install the [skills](/docs/skills/setup). This prompt turns a one-off run into a repeatable one and produces the trace.

```text
Using the SEOShark MCP on project [name]:
...
A file, usually SKILL.md, that tells an AI assistant how to do one job: which tools to call in what order, what to check, and what the output should look like. SEOShark ships skills for keyword research, competitor analysis, site audit, local SEO, link prospecting, reporting and project setup, so the assistant runs each workflow the same way every time.
...
A plain-text store it reads before it starts: markdown files of decisions, transcripts and rules, or a structured project context like SEOShark's, which holds goal, positioning, competitors, key pages and a research log. Feedback written into that store becomes a rule; feedback given once in chat is forgotten.
...
### Does SEOShark have project memory?
