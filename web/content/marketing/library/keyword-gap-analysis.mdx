...
![A ranked-keyword list showing 457 estimated organic traffic and 79 organic keywords, where the top rows are all brand terms containing the company's own name](/library/competitive-analysis/brand-dominated-ranked-keywords-seoshark.png)
...
## How to run a keyword gap in SEOShark

SEOShark does not have a one-click "keyword gap" report that takes two domains and returns the difference. What it has are the two halves: you pull ranked keywords for each domain and the agent performs the join. In practice this is more flexible than a fixed report, because you control the filters on both sides, but it is worth knowing before you go looking for a button.
...
```text
Using the SEOShark MCP, run a keyword gap between [mydomain.com]
and [competitor.com].
...
The discovery half costs nothing: read the competitor's pages, note what they cover that you do not, and check your own Search Console for questions you already receive impressions for. The part that costs money is the ranked-keyword data on the competitor's side, which is why quality SEO data sits behind a subscription in every tool. SEOShark is free to start, and paid plans include usage credits.
