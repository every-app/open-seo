...
The practical version of this check is two columns in the pages report. If `fetchClass` is anything other than `ok`, or if a page you know has 2,000 words comes back with a word count of zero, the crawl is describing the edge and not the site. [When your own crawler gets blocked](/library/site-audit/when-your-crawler-gets-blocked) covers what to do about it. Chris's episode on patents and the agentic web is [here](https://unscriptedseo.com/chris-green-do-googles-patents-still-hold-up-in-an-agentic-web/).
