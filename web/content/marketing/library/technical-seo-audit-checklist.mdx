...
![SEOShark site audit summary for unscriptedseo.com showing 318 pages crawled and 1,180 issues found, split into 35 critical, 642 warning and 503 info, with the Broken internal link group expanded to show its explanation, a How to fix instruction, and the affected URLs each listing its target URL and 404 status](/library/site-audit/audit-issue-how-to-fix-seoshark.png)
...
The audit engine assigns every issue one of three severities, and the split is deliberate rather than cosmetic. Of the 27 issue types SEOShark checks, only four are critical: a crawler that got blocked, a 5xx server error, a broken internal link, and a missing title tag. Fourteen are warnings and nine are informational.
...
1. Crawl. The free plan covers 50 pages per crawl, which is enough for most brochure sites; larger crawls run to 10,000 pages on paid plans.
...
The whole loop runs through the [SEOShark MCP](/docs/mcp), which means an agent can crawl, triage, and draft the tickets in one pass.
...
Partly. Google Search Console reports coverage and indexing for your own verified property at no cost, and it is the more reliable source for anything Google-specific. A crawler adds the on-page and internal-link picture that Search Console does not give you. SEOShark is free to start, with 50-page crawls on the free plan and 10,000-page crawls on paid plans.
