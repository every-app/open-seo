...
The [SEOShark MCP](/docs/mcp) connects to Claude Code, Claude Desktop, Codex and Cursor. You add one server, sign in, and the assistant gains the tools: Search Console reads, keyword metrics, SERP results, domain and backlink data, rank tracking, site audit, local rank grids. Search Console and audit reads use no credits; research calls that hit a data provider do, and each tool says so before it runs.
...
![A terminal session in Claude Code calling the SEOShark MCP get_search_console_performance tool for seoarcade.com over 28 days: the top rows are the brand name and real queries with clicks, impressions and position, and then a query that is the full text of an AI prompt beginning "you are a keyword analysis bot at an adtech company", with 31 impressions at position 10.2](/library/ai-agent-seo/mcp-search-console-query-pull-seoshark.png "Search Console through the SEOShark MCP")
...
Connect Search Console to the project and the [SEOShark MCP](/docs/mcp) to your assistant. This prompt is the first of the five above, with the filter step the row above makes necessary.
...
### Which AI assistants work with the SEOShark MCP?
