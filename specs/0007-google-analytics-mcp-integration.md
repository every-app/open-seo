...
| `get_google_analytics_key_events`            | Which outcomes organic traffic produces. Dimension: `eventName`; optional compatible landing-page breakdown. Filter `sessionDefaultChannelGroup` to `Organic Search` by default, with the same optional channel override as `get_google_analytics_page_performance`, so all-channel totals are never returned under an organic-outcomes label. Metrics: `keyEvents` and `totalUsers`. The implementation uses Data API metadata/compatibility checks and returns a clear unsupported-combination response rather than constructing arbitrary custom reports. |
